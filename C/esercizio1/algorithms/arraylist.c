...
/* Find the index of the value immediately smaller than given one */
int ArrayList_get_nearest_index(ArrayList* arraylist, 
                                void* data, 
                                int (*fptr)(void*, void*)) {
    if((*fptr)((arraylist->data)[0], data) == 1)
        return 0;

    for(int i=0; i<arraylist->size; i++) {
        //printf("i: %d\n", i);
        if((*fptr)((arraylist->data)[i], data) == 1)
            return i-1;
    }
...
int64_t* new_big_int(int64_t value) {
    int64_t* element = (int64_t*) malloc (sizeof(int64_t));
    *element = value;
